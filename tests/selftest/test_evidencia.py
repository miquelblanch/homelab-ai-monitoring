...
def _hub_systems_json(edades_s: dict[str, float]) -> dict:
    ahora_utc = evidencia.datetime.now(evidencia.ZoneInfo("UTC"))
    hub_systems = {
        nombre: (ahora_utc - evidencia.timedelta(seconds=edad)).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3] + "Z"
...
def test_hub_beszel_actual_sano_con_uno_caducado_entre_varios() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(evidencia, "BESZEL_HOSTS_JSON", hosts_json):
            estado = evidencia._hub_beszel_actual()
...
def test_resumen_por_sistema() -> None:
    ...
    resumen = evidencia._resumen_por_sistema(filas)
    ...
    resumen_vacio = evidencia._resumen_por_sistema(filas_todas_vacias)
    check("ausencia TOTAL ⇒ todos_sin_muestras=True", resumen_vacio["todos_sin_muestras"] is True)

    resumen_ninguno = evidencia._resumen_por_sistema([])
    check(
...
def test_congelar_hub_beszel_historico_distingue_none_de_lista_y_es_reproducible() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(evidencia, "_consultar_beszel_hub_todos_sistemas", return_value=filas_falsas):
            with store.connect(Path(tmp) / "diagnostico1.db") as conn:
                e1 = evidencia.congelar_hub_beszel_historico(conn, momento)
                e2 = evidencia.congelar_hub_beszel_historico(conn, momento)

        with patch.object(evidencia, "_consultar_beszel_hub_todos_sistemas", return_value=None):
            with store.connect(Path(tmp) / "diagnostico2.db") as conn:
                consulta_fallida = evidencia.congelar_hub_beszel_historico(conn, momento)
...
# ── Agentes / LaunchAgents (feature 016: specs/016-diagnostico-agentes/) ───
...
def test_agente_actual_running_idle_error_e_inexistente() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(evidencia, "LAUNCHAGENTS_RAW", raw):
            running = evidencia._agente_actual("amsterdam9.docker-monitor")
            idle = evidencia._agente_actual("amsterdam9.morning-report")
            error = evidencia._agente_actual("amsterdam9.rotate-hermes-logs")
            inexistente = evidencia._agente_actual("agente.que.no.existe")
...
def test_congelar_agente_vivo_arma_snapshot() -> None:
    with tempfile.TemporaryDirectory() as tmp_agentes, tempfile.TemporaryDirectory() as tmp_db:
        ...
        with patch.object(evidencia, "LAUNCHAGENTS_RAW", raw):
            with store.connect(_diag_db(tmp_db)) as conn:
                episodio = evidencia.congelar_agente_vivo(conn, "amsterdam9.rotate-hermes-logs")
                inexistente = evidencia.congelar_agente_vivo(conn, "agente.que.no.existe")
...
def test_latido_actual_sano_rancio_status_discrepante_ausente_e_inexistente() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(evidencia, "MONITOR_HEARTBEATS_DIR", heartbeats_dir):
            sano = evidencia._latido_actual("docker-monitor")
            rancio = evidencia._latido_actual("ha-monitor")
            status_discrepante = evidencia._latido_actual("dns-pi-monitor")
            ausente = evidencia._latido_actual("verify-backups")
            inexistente = evidencia._latido_actual("job-que-no-existe")
...
def test_congelar_latido_vivo_arma_snapshot() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_latidos, tempfile.TemporaryDirectory() as tmp_db:
        ...
        with patch.object(evidencia, "MONITOR_HEARTBEATS_DIR", heartbeats_dir):
            with store.connect(_diag_db(tmp_db)) as conn:
                episodio = evidencia.congelar_latido_vivo(conn, "docker-monitor")
                inexistente = evidencia.congelar_latido_vivo(conn, "job-que-no-existe")
